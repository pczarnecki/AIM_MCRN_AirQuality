...
state_gam <- gam(log(rt) ~s(home, k = 5)+s(pm25, k = 5)+s(med_temp, k =4) +s(med_hum, k=4), data = italy_25, family = gaussian)
...
p <- mydraw.gam(state_gam)

# customize plot labels
p[[1]] <- p[[1]] + labs(title = "% Change Mobility", x ="") + geom_rug(italy_25, mapping = aes(x = home), inherit.aes = F) + theme_bw() + theme(title = element_text(size = 22), axis.text = element_text(size = 16))
